...
class YoutubeVOS_Train(Dataset):
    def __init__(self, root, transform=None, seq_len=1, repeat_time=1, rand_gap=4, merge_prob=0.4):
        ...
        with open(self.seq_list_file) as f:
            self.ann_f = json.load(f)['videos']

        self.imglistdict = {}
        for seq_name in list(self.ann_f.keys()):
            ...
            for obj_n in obj_names:
                ...
                images.update([os.path.join(self.image_root, seq_name, f"{frame}.jpg") for frame in data[obj_n]["frames"]])
                labels.update([os.path.join(self.label_root, seq_name, f"{frame}.png") for frame in data[obj_n]["frames"]])
